...
func (service) SignUp(dto *MailRequestDto) *restErrors.RestErr {
	...
	plainTextContent := ""
	baseUrl := fmt.Sprintf("%s/confirm-email?email=%s&token=%s", config.Environment.FrontBaseURL, dto.Email, dto.Token)
...
func (service) ForgetPassword(dto *MailRequestDto) *restErrors.RestErr {
	...
	to := mail.NewEmail(greeting, dto.Email)
	plainTextContent := ""
	baseUrl := fmt.Sprintf("%s/reset-password?email=%s&token=%s", config.Environment.FrontBaseURL, dto.Email, dto.Token)
...
func (service) WorkspaceInvitation(dto *WorkspaceInvitationMailRequestDto) *restErrors.RestErr {
	...
	to := mail.NewEmail(greeting, dto.Email)
	plainTextContent := ""
	baseUrl := fmt.Sprintf("%s/workspaces/%s", config.Environment.FrontBaseURL, dto.WorkspaceId)
